...
import { OrganizationSuspensionDto } from '../organization/dto/organization-suspension.dto'
...
describe('global ValidationPipe — mass-assignment prevention', () => {
  ...
  describe('does NOT strip legitimate fields on the newly-validated DTOs (regression guard)', () => {
    it('UpdateOrganizationQuotaDto keeps its numeric fields', async () => {
      const out = await pipe.transform(
        { maxCpuPerSandbox: 8, snapshotQuota: 100, sandboxCreateRateLimit: 5, maxConcurrentSnapshotProcessing: 4 },
        body(UpdateOrganizationQuotaDto),
      )
      expect(out).toMatchObject({
        maxCpuPerSandbox: 8,
        snapshotQuota: 100,
        sandboxCreateRateLimit: 5,
        maxConcurrentSnapshotProcessing: 4,
      })
    })
    ...
    it('OrganizationSuspensionDto allows an omitted reason (reason is optional)', async () => {
      const out = await pipe.transform({ until: '2026-01-01T00:00:00.000Z' }, body(OrganizationSuspensionDto))
