...
export class OrganizationService implements OnModuleInit, TrackableJobExecutions, OnApplicationShutdown {
  ...
  async updateQuota(organizationId: string, updateDto: UpdateOrganizationQuotaDto): Promise<void> {
    ...
    organization.maxSecretsPerSandbox = updateDto.maxSecretsPerSandbox ?? organization.maxSecretsPerSandbox
  ...
  async updateRegionQuota(
    ...
    let regionQuota = await this.regionQuotaRepository.findOne({ where: { organizationId, regionId, sandboxClass } })
    if (!regionQuota) {
      // If region quota doesn't exist, create a new one with default quotas
      regionQuota = new RegionQuota({
        organizationId,
        regionId,
        sandboxClass,
        totalCpuQuota: 0,
