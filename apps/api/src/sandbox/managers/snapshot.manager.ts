          ...
          regions.map(async (region) => {
            ...
            return Promise.allSettled(
              runners.map(async (runner) => {
                ...
                const dockerRegistry = await this.dockerRegistryService.findOneBySnapshotImageName(snapshot.ref)
                return this.pullSnapshotRunnerWithRetries(runner, snapshot.ref, dockerRegistry)
              }),
            )
          }),
        )
