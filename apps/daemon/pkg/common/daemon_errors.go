...
// HTTPStatusCode stays 400 (not the semantically nicer 409): prod has always
// returned 400 here and existing clients depend on it. The machine code is
// the additive signal.
func (e *RecordingStillActiveError) HTTPStatusCode() int { return http.StatusBadRequest }
...
// HTTPStatusCode stays 400 (not 503): prod has always returned 400 here and
// the fix is user-actionable (install ffmpeg in the sandbox), so retry
// semantics of 503 would mislead. The machine code is the additive signal.
func (e *RecordingFfmpegNotFoundError) HTTPStatusCode() int { return http.StatusBadRequest }
