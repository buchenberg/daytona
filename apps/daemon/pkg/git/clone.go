...
func (s *Service) CloneRepository(repo *gitprovider.GitRepository, auth *http.BasicAuth, insecureSkipTLS bool, depth int) error {
	...
	// A shallow clone can omit the target commit when it predates the shallow
	// boundary, making the checkout below fail; only apply depth when cloning a
	// branch/tag tip, never for a specific commit.
	if depth > 0 && repo.Target != gitprovider.CloneTargetCommit {
