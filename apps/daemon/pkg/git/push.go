...
func (s *Service) Push(auth *http.BasicAuth, remote, branch string, setUpstream bool) error {
	if err := assertSafeRefArg("remote", remote); err != nil {
		return err
	}
	if err := assertSafeRefArg("branch", branch); err != nil {
		return err
	}
...
// assertSafeRefArg rejects a value git would treat as an option rather than a
// positional argument, blocking argument injection through a user-controlled
// remote name or branch (e.g. "--dry-run", "--receive-pack=...").
func assertSafeRefArg(kind, value string) error {
	if strings.HasPrefix(value, "-") {
		return fmt.Errorf("%w: %s %q must not start with '-'", ErrInvalidArgument, kind, value)
	}
	return nil
}
