...
func DownloadFile(c *gin.Context) {
	...
	// Open before setting headers: os.Stat only needs parent-dir traverse
	// permission, so a real read failure would otherwise surface from c.File as
	// a bare "403 Forbidden" after headers, bypassing the error envelope.
	...
	c.Header("Content-Disposition", fmt.Sprintf(`attachment; filename="%s"; filename*=utf-8''%s`,
		latin1QuotedFilename(filename), encodeRFC5987(filename)))
