...
export function SandboxTableActions({
  ...
  const isPaused = sandbox.state === SandboxState.PAUSED
  const isPausing = sandbox.state === SandboxState.PAUSING
  const isResuming = sandbox.state === SandboxState.RESUMING
  const isRecoverableError = sandbox.state === SandboxState.ERROR && sandbox.recoverable

  const canStopSandbox = writePermitted && (isStarted || isPaused)
  const canPauseSandbox = writePermitted && isVmSandbox && isStarted
  const canStartSandbox = writePermitted && (isStopped || isArchived || isPaused)
  ...
  const hasLifecycleActions =
    canStopSandbox || canPauseSandbox || canStartSandbox || canRecoverSandbox || canArchiveSandbox
      ...
      {showWriteControls && (
        <>
          ...
          >
            ...
            ) : isStarting || isStopping || isPausing || isResuming ? (
