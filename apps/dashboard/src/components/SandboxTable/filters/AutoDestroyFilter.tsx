...
import { CalendarIcon, TimerOff } from 'lucide-react'

interface AutoDestroyFilterProps {
  value: (Date | undefined)[]
  onFilterChange: (value: (Date | undefined)[] | undefined) => void
}

export function AutoDestroyFilterIndicator({ value, onFilterChange }: AutoDestroyFilterProps) {
  ...
  return (
    <FacetedFilterRoot
      title="Auto Destroy"
      hasValue={selectedDates.length > 0}
      onClear={() => onFilterChange(undefined)}
    >
      <FacetedFilterAnchor>
        <FacetedFilterLabelTrigger icon={<TimerOff />} aria-label="Filter by Auto Destroy">
          Auto Destroy
        </FacetedFilterLabelTrigger>
        <FacetedFilterOperator />
        <FacetedFilterValueTrigger className="px-1" aria-label="Edit Auto Destroy filter">
          <FacetedFilterValues title="Auto Destroy" items={selectedDates} maxValues={1} />
        </FacetedFilterValueTrigger>
        <FacetedFilterClear aria-label="Clear Auto Destroy filter" />
      </FacetedFilterAnchor>
      <FacetedFilterContent className="p-3 w-auto">
        <AutoDestroyFilter onFilterChange={onFilterChange} value={value} />
      </FacetedFilterContent>
    </FacetedFilterRoot>
  )
}

interface AutoDestroyFilterContentProps {
  onFilterChange: (value: (Date | undefined)[] | undefined) => void
  value: (Date | undefined)[]
}

export function AutoDestroyFilter({ onFilterChange, value }: AutoDestroyFilterContentProps) {
