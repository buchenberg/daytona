...
```ruby
def experimental_create_snapshot(name:, timeout:, include_memory:)
...
- `timeout` _Numeric_ - Maximum wait time in seconds (defaults to 60 s)
- `include_memory` _Boolean_ - Include VM memory in the snapshot. VM sandboxes only (Windows, Linux VM)
