...
| **Sandbox types**                                                                                                                   |
| ----------------------------------------------------------------------------------------------------------------------------------- |
| <u>[**Linux Container**](#create-sandboxes)</u><br />Default Linux container runtime.                                               |
| <u>[**Linux VM**](#linux-vm-sandboxes)</u><br />Linux guest in a virtual machine for running Linux-specific tools and workflows.    |
| <u>[**Windows**](#windows-sandboxes)</u><br />Windows OS runtime in a virtual machine for running Windows applications and tooling. |
| <u>[**GPU**](#gpu-sandboxes)</u><br />NVIDIA GPU for model inference, fine-tuning, and CUDA-accelerated compute.                    |
...
4. Specify the **`GPU type`**(s): **`H100`** **`H200`** **`RTX-PRO-6000`** **`RTX-4090`** **`RTX-5090`**
...
VM sandboxes run in a virtual machine and use the Linux and Windows sandbox classes. VM sandboxes support VM-only capabilities such as [pause/resume](#pause-sandboxes) and [hot/cold snapshots](#create-snapshot-from-sandbox).

- [Linux VM](#linux-vm-sandboxes): **`linux-vm`**
- [Windows](#windows-sandboxes): **`windows`**

:::note[Current limitation]
VM sandboxes can currently only be created from existing [snapshots](/docs/en/snapshots). Dynamic builds through the [declarative builder](/docs/en/declarative-builder) are supported for container sandboxes only.
:::
...
1. Create a [Linux VM snapshot](/docs/en/snapshots#linux-vm-snapshots)
...
# 2. Create a VM sandbox from the snapshot
daytona create --snapshot my-vm-snapshot --target us-west-2
```
...
Pausing a sandbox preserves filesystem state and memory, and does not consume CPU. To resume a paused sandbox, [start](#start-sandboxes) it again; processes continue from where they were paused.
...
The snapshot inherits the source sandbox's class: a container sandbox produces a [container snapshot](/docs/en/snapshots#create-snapshots) and a VM sandbox produces a [VM snapshot](/docs/en/snapshots#vm-snapshots). The resulting snapshot can only create sandboxes of that same class.
...
// Hot snapshot (filesystem and memory, sandbox running)
await sandbox._experimental_createSnapshot("my-sandbox-snapshot", 60, true);
```
...
# Hot snapshot (filesystem and memory, sandbox running)
sandbox.experimental_create_snapshot(name: "my-sandbox-snapshot", include_memory: true)
```
...
// Hot snapshot (filesystem and memory, sandbox running)
err = sandbox.ExperimentalCreateSnapshotWithMemory(ctx, "my-sandbox-snapshot", 60*time.Second)
if err != nil {
    return err
}
```
...
// Hot snapshot (filesystem and memory, sandbox running)
sandbox.experimentalCreateSnapshot("my-sandbox-snapshot", 60, true);
```
...
```bash
# Set "includeMemory": true for a hot snapshot (sandbox must be running)
...
| Snapshotting      | The sandbox is creating a [**snapshot**](/docs/en/snapshots) of its filesystem and memory.  |
