...
| **Snapshot type**                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <u>[**Linux Container**](#create-snapshots)</u><br />Used to create <u>[**container sandboxes**](/docs/en/sandboxes#create-sandboxes)</u>.                                                                                         |
| <u>[**Linux VM**](#linux-vm-snapshots)</u><br />Used to create <u>[**Linux VM sandboxes**](/docs/en/sandboxes#linux-vm-sandboxes)</u>.                                                                                             |
| <u>[**Windows**](#windows-snapshots)</u><br />Used to create <u>[**Windows sandboxes**](/docs/en/sandboxes#windows-sandboxes)</u>.                                                                                                 |
| <u>[**From sandbox**](/docs/en/sandboxes#create-snapshot-from-sandbox)</u><br />A snapshot <u>[**captured from an existing sandbox**](/docs/en/sandboxes#create-snapshot-from-sandbox)</u>, inheriting the source sandbox's class. |

## Create Snapshots

Daytona provides methods to create snapshots.

- [GPU snapshots](#gpu-snapshots) (for [GPU sandboxes](/docs/en/sandboxes#gpu-sandboxes))
- [VM snapshots](#vm-snapshots) (for [VM sandboxes](/docs/en/sandboxes#vm-sandboxes))
...
6. Specify the **`GPU type`**: **`NVIDIA H100`** **`NVIDIA H200`** **`NVIDIA RTX PRO 6000`** **`NVIDIA RTX 4090`** **`NVIDIA RTX 5090`**
...
Daytona provides methods to create VM snapshots.

VM snapshots are used to create [VM sandboxes](/docs/en/sandboxes#vm-sandboxes). A VM snapshot is bound to a sandbox class. The class is set when the snapshot is created and determines which sandbox type the snapshot can produce. Daytona resolves the class from the snapshot at sandbox creation time, so you do not pass a class when creating the sandbox. VM snapshots are distinct from container snapshots and cannot be used to create container sandboxes.

- [Linux VM](#linux-vm-snapshots): **`linux-vm`**
- [Windows](#windows-snapshots): **`windows`**

A VM snapshot can also be [captured from an existing sandbox](/docs/en/sandboxes#create-snapshot-from-sandbox) as either a **cold snapshot** (filesystem only) or a **hot snapshot** (filesystem and memory).

#### Linux VM Snapshots

Daytona provides methods to create Linux VM snapshots.

1. Create a snapshot from a base image
2. Set the snapshot's sandbox class to **`linux-vm`**
3. Target the **`us-west-2`** region
4. Create a snapshot from the base image

<Tabs syncKey="language">
<TabItem label="Python" icon="seti:python">

```python
from daytona import Daytona, DaytonaConfig, CreateSnapshotParams, SandboxClass
...
const daytona = new Daytona({ target: "us-west-2" });
const snapshot = await daytona.snapshot.create({
  name: "my-vm-snapshot",
  image: "ubuntu:22.04",
...
config = Daytona::Config.new(target: 'us-west-2')
daytona = Daytona::Daytona.new(config)
snapshot = daytona.snapshot.create(
  Daytona::CreateSnapshotParams.new(
    name: 'my-vm-snapshot',
...
```bash
daytona snapshot create my-vm-snapshot --image ubuntu:22.04 --sandbox-class linux-vm --region us-west-2
```
...
Daytona provides methods to create Windows VM snapshots.

Windows VM snapshots are used to create [Windows VM sandboxes](/docs/en/sandboxes#windows-sandboxes). They cannot be created from a base image. They are produced only through the [snapshot-from-sandbox](/docs/en/sandboxes#create-snapshot-from-sandbox) flow: start from an existing [Windows sandbox](/docs/en/sandboxes#windows-sandboxes) and capture its current state as a snapshot.
...
Daytona supports creating snapshots from images from [Docker Hub](#docker-hub), [Google Artifact Registry](#google-artifact-registry), [GitHub Container Registry](#github-container-registry), [Amazon ECR](#amazon-elastic-container-registry) or other private container registries.
