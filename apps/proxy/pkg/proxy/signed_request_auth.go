...
// the given canonical string. Key rotation is handled by the API push-evicting this proxy's
// signing key cache (ProxyCacheInvalidationService), so a mismatch is simply rejected.
