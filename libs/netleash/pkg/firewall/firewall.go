...
func (fw *Firewall) Adopt() error {
	...
	for _, name := range []string{pinAllowedDomains, pinAllowedWildcards, pinAllowedIps, pinInternalZones, pinEvents, pinProxyConfig} {
		...
		if err != nil {
			// proxy_config was added after pinning shipped; a pin set written by an
			// older version doesn't have it (and its programs have no proxy gate to
			// configure), so its absence must not make the whole pin set
			// un-adoptable — that would tear down a working filter.
			if name == pinProxyConfig && os.IsNotExist(err) {
				fw.log.Warn("pinned firewall predates proxy enforcement; egress-proxy gating unavailable until it is rebuilt")
				continue
			}
			fw.Cleanup()
