...
func (m *Manager) configure(id string, target attachTarget, domains []string, enforceProxy bool) error {
	...
	if ok {
		...
		// (Re-)apply the web-port proxy gate. An adopted entry starts with
		// enforceProxy=false even though its pinned gate may be live; writing the
		// desired state explicitly is idempotent and makes the pinned map converge
		// on the caller's intent. Enforcement failure must not undo the domain
		// update above — the IP allow list still enforces — so it is logged, not
		// returned.
		existing.enforceProxy = enforceProxy
		m.applyProxyEnforcementLocked(id, existing)
		return nil
	}
	...
	fw := firewall.New(firewall.Config{
		...
		// Gate web ports through the proxy only when it is actually running;
		// otherwise the requested enforcement is recorded on the entry and applied
		// when EnableEgressProxy brings the proxy up.
		EnforceProxy: enforceProxy && m.egressProxyAddr != "",
	...
	if enforceProxy && m.egressProxyAddr == "" {
		m.log.Warn("netleash: proxy enforcement requested but the egress proxy is not running; web-port gating deferred until EnableEgressProxy", "workload", id)
	}
	m.log.Info("domain allow list applied", "workload", id, "domains", norm, "interface", target.iface, "enforce_proxy", enforceProxy && m.egressProxyAddr != "")
	return nil
}
...
// running, off otherwise. Failures are logged (not returned) — enforcement is a
// hardening layer on top of the still-active IP allow list. Caller must hold
// m.mu.
func (m *Manager) applyProxyEnforcementLocked(id string, e *entry) {
	effective := e.enforceProxy && m.egressProxyAddr != ""
	if err := e.fw.SetProxyEnforcement(m.egressProxyAddr, effective); err != nil {
		m.log.Error("netleash: failed to apply proxy enforcement; web-port gating inactive for this workload",
			"workload", id, "enforce", effective, "error", err)
		return
	}
...
func (m *Manager) EnableEgressProxy(cfg EgressProxyConfig) (*EgressProxyHandle, error) {
	...
	for id, e := range m.entries {
		...
		if e.enforceProxy {
			m.applyProxyEnforcementLocked(id, e)
		}
	}
	m.mu.Unlock()
